...
import { BookOpen, Camera, CameraOff, Layers, ShoppingBag, User } from "lucide-react";
          ...
          >
            {technicalSkills.map((skill, index) => (
              <motion.div key={skill.name} variants={fadeInUp} custom={index}>
                ...
                <Progress value={skill.percentage} className="h-2 bg-card" indicatorClassName="bg-primary" />
              </motion.div>
            ))}
          </motion.div>
        </motion.div>
